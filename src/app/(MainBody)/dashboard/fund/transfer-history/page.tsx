...
const DepositHistory = () => {
  ...
  const filteredTx = useMemo(() => {
    ...
    return fundTransferHistory.filter((tx: IFundTransaction) =>
      [
        filterCode(tx.uCode),
  ...
  }, [fundTransferHistory, debouncedFilterText]);
  ...
  const columns: TableColumn<IFundTransaction>[] = useMemo(
    () => [
      ...
      },
      {
        name: "Receiver",
        selector: (row) => {
          const tx = row.txUCode;
          const uc = row.uCode;

          if (typeof tx === "object" && tx !== null) {
            return tx.username || tx.name || "N/A";
          }

          if (typeof uc === "object" && uc !== null) {
            return uc.username || uc.name || "N/A";
          }
        ...
        cell: (row) => {
          ...
          if (typeof tx === "object" && tx !== null) {
            displayValue = tx.username || tx.name || "N/A";
          } else if (typeof uc === "object" && uc !== null) {
            displayValue = uc.username || uc.name || "N/A";
          }
